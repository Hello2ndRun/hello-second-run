...
function ImpactCard({ icon: Icon, value, label, accent = false }: {
  ...
  return (
    <div className={`p-5 border ${accent ? 'bg-gradient-to-br from-red-50 to-pink-50 border-red-200' : 'bg-[#ffffff] border-gray-100'}`}>
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${accent ? 'bg-red-100' : 'bg-[#111113]/10'}`}>
        <Icon size={20} className={accent ? 'text-red-500' : 'text-[#111113]'} />
      </div>
      <p className="text-2xl font-black text-[#111113]">{value}</p>
...
function PartnerCard({ partner, onEdit }: { partner: DonationPartner; onEdit: (p: DonationPartner) => void }) {
  ...
  return (
    <div className="border border-gray-100 bg-[#ffffff] hover:border-[#e4e4e7] transition-all">
      <div className="p-4 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-black text-sm text-[#111113]">{partner.name}</h3>
        ...
        <div className="px-4 pb-4 pt-0 border-t border-gray-50 space-y-3">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p className="font-medium text-[#111113]">{partner.kontaktperson}</p>
            ...
            <div>
              ...
              <p className="font-medium text-[#111113]">{partner.maxKapazitaet}</p>
...
function NewPartnerModal({ onClose, onSave }: { onClose: () => void; onSave: (data: Omit<DonationPartner, 'id' | 'createdAt'>) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#ffffff] max-w-lg w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-black text-[#111113] mb-1">Neuer Spendenpartner</h2>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="z.B. Tafel Salzburg" className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Organisation *</label>
              <input value={form.organisation} onChange={e => setForm({ ...form, organisation: e.target.value })} placeholder="z.B. Tafel Österreich" className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.kontaktperson} onChange={e => setForm({ ...form, kontaktperson: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">E-Mail *</label>
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
          </div>

          <div>
            <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Telefon</label>
            <input value={form.telefon} onChange={e => setForm({ ...form, telefon: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
          </div>

          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">PLZ</label>
              <input value={form.plz} onChange={e => setForm({ ...form, plz: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Ort</label>
              <input value={form.ort} onChange={e => setForm({ ...form, ort: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Land</label>
              <select value={form.land} onChange={e => setForm({ ...form, land: e.target.value })} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none bg-[#ffffff]">
          ...
          <div>
            ...
            <input value={form.maxKapazitaet} onChange={e => setForm({ ...form, maxKapazitaet: e.target.value })} placeholder="z.B. 10 Paletten/Woche" className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none" />
          </div>

          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.kuehlung} onChange={e => setForm({ ...form, kuehlung: e.target.checked })} className="accent-[#111113]" />
              <Snowflake size={14} className="text-blue-400" />
              <span className="text-sm">Kühlware möglich</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.abholung} onChange={e => setForm({ ...form, abholung: e.target.checked })} className="accent-[#111113]" />
          ...
          <div>
            ...
            <textarea value={form.notizen} onChange={e => setForm({ ...form, notizen: e.target.value })} rows={2} className="w-full border border-gray-200 p-2.5 text-sm mt-1 focus:border-[#111113] focus:outline-none resize-none" />
          </div>
        </div>

        <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
          <button onClick={onClose} className="px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-400 hover:text-gray-600">
            Abbrechen
          </button>
          <button onClick={handleSubmit} disabled={!form.name || !form.organisation || !form.email} className="px-5 py-2.5 bg-[#111113] text-[#ffffff] text-[10px] font-semibold uppercase tracking-[0.08em] hover:bg-[#8cc63f] hover:text-[#111113] transition-all disabled:opacity-40 disabled:cursor-not-allowed">
    ...
    <div>
      ...
      {activeTab === 'overview' && impact && (
        <div className="space-y-6">
          {/* Impact Hero */}
          <div className="bg-gradient-to-br from-[#111113] to-[#111113] text-white p-8 relative overflow-hidden">
            <div className="absolute top-4 right-4 text-6xl opacity-10">❤️</div>
            <p className="text-[10px] font-black uppercase tracking-[0.08em] text-[#8cc63f] mb-2">Social Impact</p>
            ...
            <div>
              ...
              <div className="space-y-2">
                {impact.letzteSpenden.map(d => {
                  ...
                  return (
                    <div key={d.id} className="flex items-center justify-between bg-[#ffffff] border border-gray-100 p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-red-50 rounded-lg flex items-center justify-center flex-shrink-0">
                          <Heart size={14} className="text-red-400" />
                        </div>
                        <div>
                          <p className="text-sm font-bold text-[#111113]">{partner?.name || 'Unbekannt'}</p>
                          <p className="text-xs text-gray-400 truncate max-w-[300px]">{d.artikelBeschreibung}</p>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-sm font-black text-[#111113]">{d.gewichtKg} kg</p>
        ...
        <div className="space-y-3">
          {partners.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button onClick={() => setShowNewPartner(true)} className="mt-4 inline-flex items-center gap-2 bg-[#111113] text-[#ffffff] px-5 py-2.5 font-black uppercase tracking-[0.1em] text-[11px] hover:bg-[#8cc63f] hover:text-[#111113] transition-all">
        ...
        <div className="space-y-3">
          {records.length === 0 ? (
            ...
            records.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(d => {
              const partner = donationPartnersCollection.getById(d.donationPartnerId);
              return (
                <div key={d.id} className="bg-[#ffffff] border border-gray-100 p-5">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <h3 className="font-bold text-sm text-[#111113]">{partner?.name || 'Unbekannt'}</h3>
                      <p className="text-xs text-gray-400 mt-1">{d.artikelBeschreibung}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-black text-[#111113]">{d.gewichtKg} kg</p>
                  ...
                  {d.status !== 'bestaetigt' && (
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
                      {d.status === 'vorgeschlagen' && (
                        <button onClick={() => updateDonationStatus(d.id, 'geplant')} className="text-[10px] font-semibold uppercase tracking-[0.08em] px-3 py-1.5 border border-blue-200 text-blue-600 hover:bg-blue-50 transition-all">
                          <Clock size={11} className="inline mr-1" /> Als geplant markieren
                        </button>
                      )}
                      {(d.status === 'geplant' || d.status === 'vorgeschlagen') && (
                        <button onClick={() => handleMarkAbgeholt(d.id)} className="text-[10px] font-semibold uppercase tracking-[0.08em] px-3 py-1.5 border border-purple-200 text-purple-600 hover:bg-purple-50 transition-all">
                          <Truck size={11} className="inline mr-1" /> Abgeholt
                        </button>
                      )}
                      <button onClick={() => handleConfirm(d.id)} className="text-[10px] font-semibold uppercase tracking-[0.08em] px-3 py-1.5 bg-green-600 text-white hover:bg-green-700 transition-all">
